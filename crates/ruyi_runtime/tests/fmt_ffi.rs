 ...
 *
 * `__string_replace_all` writes a transformed copy of an input buffer
 ...
 * @date 2026-07-11
 */
use ruyi_runtime::fmt_ffi::__string_replace_all;

/// `__string_replace_all("a.b.c", ".", "/")` MUST yield `"a/b/c"`.
...
/// `__string_replace_all("hello", "x", "y")` MUST return the input
...
/// `__string_replace_all("abc", "b", "")` MUST yield `"ac"`. The
/// replacement target has length zero, exercising the delete branch.
#[test]
fn test_string_replace_all_empty_to() {
    let input: &[u8] = b"abc";
    let from: &[u8] = b"b";
    let to: &[u8] = b"";
    let mut out = [0u8; 16];
