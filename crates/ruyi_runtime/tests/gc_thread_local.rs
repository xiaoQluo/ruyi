...
use ruyi_runtime::gc_exports::{
    ruyi_gc_alloc, ruyi_gc_collect, ruyi_gc_add_root, ruyi_gc_remove_root,
};
...
fn test_thread_local_allocation_unique_pointers() {
    ...
    assert_ne!(main_ptr, thread_ptr, "allocations from different threads should have unique pointers");
}
...
fn test_thread_local_collect_isolation() {
    ...
    unsafe {
        assert_eq!(*(main_obj1 as *mut u64), 0xDEADBEEF, "rooted object should survive");
...
fn test_concurrent_thread_allocations() {
    ...
    assert_eq!(total, NUM_THREADS * ALLOCS_PER_THREAD, "all allocations should succeed");
}
...
fn test_thread_exit_does_not_corrupt_main_heap() {
    ...
    let main_ptr = ruyi_gc_alloc(64);
    assert!(!main_ptr.is_null(), "main thread allocation should work after thread exit");
    
    unsafe {
        *(main_ptr as *mut u64) = 0x12345678;
        ruyi_gc_collect();
        assert_eq!(*(main_ptr as *mut u64), 0x12345678, "main heap should not be corrupted");
    }
}
...
fn test_cross_thread_pointer_not_accessible() {
    ...
    assert!(ptr_in_thread != 0, "thread should return valid pointer address");
}
    ...
    for thread_id in 0..NUM_THREADS {
        let handle = thread::spawn(move || {
            ...
            for i in 0..ITERATIONS {
                ...
                assert!(!obj.is_null(), "thread {}: allocation {} failed", thread_id, i);
            ...
            for (i, &obj) in roots.iter().enumerate() {
                ...
                unsafe {
                    assert_eq!(*(obj as *mut u64), expected, 
                        "thread {}: root {} value mismatch", thread_id, i);
                }
            }
