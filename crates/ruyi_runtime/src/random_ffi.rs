 ...
 *
 * - `ruyi_random_new(seed)` — initialize a generator; `seed == 0` falls back
 *   to an entropy-derived seed from the runtime `RandomState`.
 * - `ruyi_random_int(rng, min, max)` — uniform integer in the closed
 *   interval `[min, max]`. Returns `min` when `min === max`. Aborts on
 *   `min > max` so callers see a diagnostic rather than silently wrapping.
 * - `ruyi_random_float(rng)` — uniform float in `[0.0, 1.0)`.
 * - `ruyi_random_bool(rng)` — uniform boolean derived from `ruyi_random_int`.
 * - `ruyi_random_bytes(rng, n)` — `n` pseudo-random bytes returned as a
...
#[no_mangle]
pub extern "C" fn ruyi_random_new(seed: i64) -> i64 {
    let s = if seed == 0 {
...
#[no_mangle]
pub extern "C" fn ruyi_random_int(rng: i64, min: i64, max: i64) -> i64 {
    if min == max {
        return min;
    }
    if min > max {
...
#[no_mangle]
pub extern "C" fn ruyi_random_float(rng: i64) -> f64 {
    let mut s = (rng as u64).wrapping_mul(XSHIFT_MULT_A);
    s ^= s >> 33;
    s = s.wrapping_mul(XSHIFT_MULT_B);
    s ^= s >> 33;
...
/// Return a uniform boolean derived from `ruyi_random_int(rng, 0, 1)`.
#[no_mangle]
pub extern "C" fn ruyi_random_bool(rng: i64) -> bool {
    ruyi_random_int(rng, 0, 1) == 1
}
...
/// `ruyi_random_bytes`. Callers MUST copy the bytes out (e.g. into a
...
#[no_mangle]
pub extern "C" fn ruyi_random_bytes(rng: i64, n: i64) -> *mut i8 {
...
mod tests {
    ...
    fn test_new_distinct_seeds() {
        let a = ruyi_random_new(1);
        let b = ruyi_random_new(2);
        assert_ne!(a, b);
    }

    #[test]
    fn test_new_zero_uses_entropy() {
        let a = ruyi_random_new(0);
        let b = ruyi_random_new(0);
        assert_ne!(a, b);
    }
    ...
    fn test_int_range() {
        let seed = 42_i64;
        for _ in 0..10_000 {
            let v = ruyi_random_int(seed, 5, 10);
    ...
    fn test_int_min_eq_max() {
        for seed in [0_i64, 1, 42, i64::MAX] {
            assert_eq!(ruyi_random_int(seed, 5, 5), 5);
        }
    }
    ...
    fn test_bytes_payload_writable_and_nul_terminated() {
        ...
        for len in [0_i64, 1, 16, 256] {
            let p = ruyi_random_bytes(seed, len);
            assert!(!p.is_null());
            unsafe {
    ...
    #[test]
    #[ignore = "ruyi_random_int aborts (not panics) on min > max to avoid FFI UB"]
    fn test_int_aborts_when_min_greater_than_max() {
        let _ = ruyi_random_int(0, 10, 5);
    }
}