...
| **类型检查器** | ~75% | Trait 约束未执行；超特质未检查；`impl Trait for Type` 不完整 |
| **代码生成** | ~45% | **不支持成员访问（`obj.prop`）、数组/对象字面量、BigInt/模板字符串、for/for-in/for-of、try/catch、break/continue、类布局** |
| **宏展开** | ~60% | 复杂重复模式、卫生性边界情况 |
| **驱动器** | ~80% | 运行时未链接；模块系统内联而非正式导入 |
| **GC** | 编译器端存根 / 运行时 85% | 编译器端 GC 为存根；运行时 GC 未对接代码生成 |
| **运行时** | 编译器端 30% / 运行时库 70% | **`ruyi_await` 为空操作；async 同步运行；运行时无 stdlib 类型** |
